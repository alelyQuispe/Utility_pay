...
import contractABI from 'src/app/abi/UtilityPay.json';
...
export class ServiciosListComponent implements OnInit {
  ...
  async onSubmit(event: Event) {
    ...
    // Validar que la dirección de servicio sea válida
    if (!this.web3?.utils.isAddress(this.selectedService)) {
      alert('La dirección del servicio no es válida.');
      return;
    }
    ...
    try {
      ...
      const paymentValue = this.web3?.utils.toWei(
